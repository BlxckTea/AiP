...
def saveText(max_x, max_y, max_z, diff_x, diff_y, diff_z, rm_x, rm_y, rm_z, raw_x, raw_y, raw_z):
   print("### Save start ###")

   fd = open("./max_diff.txt", 'a+') # a+ : 기존데이터 그대로두고 마지막에 내용 입력
   now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
   max_data = "%.10f, " % max_x + "%.10f, " % max_y + "%.10f " % max_z
   fd.write(max_data + now + '\n')

   fr = open("./ori_diff.txt", 'a+')   
   max_data = "%.10f, " % diff_x + "%.10f, " % diff_y + "%.10f " % diff_z
   fr.write(max_data + now + '\n')
   
   fr = open("./max_raw.txt", 'a+')   
   max_data = "%.10f, " % rm_x + "%.10f, " % rm_y + "%.10f " % rm_z
   fr.write(max_data + now + '\n')
   
   fr = open("./ori_raw.txt", 'a+')   
   max_data = "%.10f, " % raw_x + "%.10f, " % raw_y + "%.10f " % raw_z
   fr.write(max_data + now + '\n')
...
while True:
   ...
   if (stop - start ) >= 10.0 and (stop - start) < 10.1:
      saveText(max_x, max_y, max_z, diff_x, diff_y, diff_z, rm_x, rm_y, rm_z, raw_x, raw_y, raw_z)
      max_x = 0.0
      max_y = 0.0
      max_z = 0.0
      old_x = read_raw_data(ACCEL_XOUT_H) / 16384.0
